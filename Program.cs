...
var projectClient = new AIProjectClient(
    new Uri(settings.ProjectEndpoint), new AzureCliCredential());
...
{
    client.BaseAddress = new Uri(builder.Configuration["Services:TemplateIntelligence:BaseUrl"]
        ?? throw new InvalidOperationException("Template Intelligence BaseUrl is missing."));
    client.Timeout = TimeSpan.FromSeconds(30);
});
builder.Services.AddHttpClient<LabelGenerationClient>(client =>
{
    client.BaseAddress = new Uri(builder.Configuration["Services:LabelGeneration:BaseUrl"]
        ?? throw new InvalidOperationException("Label Generation BaseUrl is missing."));
    client.Timeout = TimeSpan.FromMinutes(3);
});
builder.Services.AddHttpClient<CandidateEvaluationClient>(client =>
{
    client.BaseAddress = new Uri(builder.Configuration["Services:CandidateEvaluation:BaseUrl"]
        ?? throw new InvalidOperationException("Candidate Evaluation BaseUrl is missing."));
    client.Timeout = TimeSpan.FromMinutes(3);
});
builder.Services.AddHttpClient<BeamSearchClient>(client =>
{
    client.BaseAddress = new Uri(builder.Configuration["Services:BeamSearch:BaseUrl"]
        ?? throw new InvalidOperationException("Beam Search BaseUrl is missing."));
